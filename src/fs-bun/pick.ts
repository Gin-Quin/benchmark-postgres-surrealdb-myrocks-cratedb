...
const filePath = (id: string) => `${volume}/${id}.bun`
...
now = Date.now()
await Promise.all(ids.map(id => Bun.file(filePath(id)).arrayBuffer()))
console.log("(Bun.file) Pick author by id:", Date.now() - now, "ms")
