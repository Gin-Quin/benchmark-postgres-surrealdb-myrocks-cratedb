...
const filePath = `${volume}/fs-bun.file`
...
now = Date.now()
await Promise.all(data.map(record => Bun.write(filePath, record.value)))
console.log("(Bun.write) Update author by id:", Date.now() - now, "ms")
